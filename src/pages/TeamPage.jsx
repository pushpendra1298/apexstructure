...
const team = [
  ...
  { name: 'Anil Rajpoot', role: 'Billing and Estimation', image: './anil.jpg' },
      ...
      <section className="relative pt-36 pb-20 overflow-hidden"
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.7 }}
            ...
            Meet Our <span style={gradText}>Exceptional Team</span>
          </motion.h1>
      ...
      <section className="relative py-24 overflow-hidden" style={{ background: 'linear-gradient(180deg, #030812 0%, #06101f 50%, #030812 100%)' }}>
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {team.map((m, i) => (
              <motion.div key={m.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.6 }}
                className="group rounded-3xl overflow-hidden relative" style={glassCard}>
                <div className="h-96 overflow-hidden relative">
                  <img src={m.image} alt={m.name} className="w-full h-full object-cover object-top transition-transform duration-700 group-hover:scale-110" style={{ filter: 'grayscale(20%) contrast(1.1)' }} />
                  ...
                  <div className="absolute bottom-4 left-5 right-5">
                    ...
                    <p className="text-xs font-semibold tracking-wide uppercase" style={{ color: '#fb923c' }}>{m.role}</p>
                    {m.experience && <p className="text-[10px] text-white/60 uppercase tracking-widest mt-1 font-medium">{m.experience}</p>}
                  </div>
                </div>
                {m.description && <div className="p-6 pt-4"><p className="text-slate-400 text-sm leading-relaxed">{m.description}</p></div>}
              </motion.div>
            ))}
          </div>
        </div>
