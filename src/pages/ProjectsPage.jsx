...
import { Building2, ArrowLeft, Mail, Phone, MapPin, Layers, Facebook, Linkedin, Instagram } from 'lucide-react'
            ...
            <div className="flex items-start gap-4 flex-wrap">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold text-white text-sm mb-1">
                  Previous Experience as Structural Engineer ({previousExperience.period})
                </p>
                <p className="text-slate-400 text-xs mb-3">Successfully contributed to diverse industrial & civil structures:</p>
                <div className="flex flex-wrap gap-2">
                  {previousExperience.types.map(type => (
                    <span key={type} className="px-2.5 py-1 rounded-lg text-xs font-medium"
          ...
          <div className="flex gap-2 flex-wrap">
            {allCats.map(cat => (
              <button key={cat} onClick={() => setActiveCat(cat)}
                ...
                style={{
                  background: activeCat === cat ? 'linear-gradient(135deg, #f97316, #ea580c)' : 'rgba(255,255,255,0.05)',
                  color: activeCat === cat ? '#fff' : 'rgba(255,255,255,0.5)',
                  border: activeCat === cat ? 'none' : '1px solid rgba(255,255,255,0.1)',
                  boxShadow: activeCat === cat ? '0 2px 10px rgba(249,115,22,0.35)' : 'none',
                }}>
            ...
            {allTags.map(tag => {
              ...
              return (
                <button key={tag} onClick={() => setActiveTag(tag)}
                  ...
                  style={{
                    background: isActive && tc ? tc.bg : 'rgba(255,255,255,0.04)',
                    color: isActive && tc ? tc.color : 'rgba(255,255,255,0.4)',
                    border: isActive && tc ? `1px solid ${tc.border}` : '1px solid rgba(255,255,255,0.08)',
                  }}>
                  {tag}
                </button>
              )
                ...
                return (
                  <motion.div key={project.id}
                    ...
                    <div className="relative h-44 overflow-hidden flex-shrink-0">
                      ...
                      <div className="absolute top-3 left-3">
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-md"
                          ...
                          {project.category === 'Architecture & Engineering' ? 'Arch.' : 'Struct.'}
                        </span>
                      </div>
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      <h4 className="font-bold text-white text-sm mb-3 leading-snug group-hover:text-orange-100 transition-colors">
                        {project.name}
                      </h4>
                      ...
                      <div className="space-y-2 flex-1">
                        {project.area && project.area !== '—' && (
                          <div className="flex items-start gap-2 text-xs text-slate-400">
                            <Layers size={11} className="mt-0.5 flex-shrink-0" style={{ color: '#fb923c' }} />
                            <span>{project.area}</span>
                          </div>
                        )}
                        <div className="flex items-start gap-2 text-xs text-slate-400">
                          <MapPin size={11} className="mt-0.5 flex-shrink-0" style={{ color: '#fb923c' }} />
                          <span className="leading-relaxed">{project.location}</span>
                        </div>
                        {project.client && project.client !== '—' && (
                          <div className="flex items-start gap-2 text-xs text-slate-500">
                            <Building2 size={11} className="mt-0.5 flex-shrink-0" style={{ color: 'rgba(249,115,22,0.5)' }} />
                            <span className="leading-relaxed">{project.client}</span>
                          </div>
                        )}
                        {project.contractor && (
                          <div className="flex items-start gap-2 text-xs text-slate-600">
                            <span className="mt-0.5 w-2.5 flex-shrink-0 text-center" style={{ color: 'rgba(255,255,255,0.3)' }}>↪</span>
                            <span className="leading-relaxed italic">{project.contractor}</span>
                          </div>
                        )}
                      </div>
